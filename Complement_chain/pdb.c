...
void BestDiag(double **measures, unsigned int n, unsigned int m, double *S_max,
		...
		unsigned int *j_start, unsigned int *i_max_measure,
		unsigned int *j_max_measure){

//n for n_P1; m for n_P2
//i_max, j_max for (i,j)-pair with max S-sum
//i_max_measure, j_max_measure for (i,j)-pair with max measure

int d; //diagonal number: [-n_P1+1; n_P2+1]
unsigned int i,j, max_i, max_j, max_measure_i, max_measure_j;
double **S; //S-table
...
for (d=-n+1; d<=(int)m-1; d++){ //(int) needs to compare int and unsigned int
	for (i = ((-d+1 > 1) ? -d+1 : 1); i <= ((m-d < n) ? m-d : n); i++){
	//for i=max(-d+1, 1) to min(n_P2-d, n_p1)
		j = d+i;
		S[i][j] = measures[i][j] + ((i==1 || j==1 || i==n/2+1 || j==m/2+1) ? 0 : S[i-1][j-1]);
		if (S[i][j] < 0)  S[i][j] = 0;
		if (S[i][j] >= max_sum){
			max_sum = S[i][j];
			max_i=i; max_j=j;
...
printf("S max: %lg, i_start: %d, j_start: %d, i_max: %d, j_max: %d, max_M: %lg\n", max_sum, i+1, j+1, max_i, max_j, measure_max);
//Enable in test mode
						  ...
						  char chainname)
{
  ...
  Cvy = ort(Cvy);
  /* Cvy = fromto(Pp, Cpy);	// P_OP1 vector
  Cvy2 = fromto(Pp, Cpy2);	// P_OP2 vector
  if ( scalar(Cvy, Cvx) <= scalar(Cvy2, Cvx) ){
    if ( scalar(Cvy, Cvy) == 0 ) {
      perror("Cvy has zero length!!!");
      exit(1);
    }
    ...
    if ( scalar(Cvy2, Cvy2) == 0 ) {
      perror("Cvy2 has zero length!!!");
      exit(1);
    }
    Cvy = ort(Cvy2);} */
...
unsigned int writetoPDB(char *filename, 
  ...
  for (i=1; i<=natoms; i++){
    fprintf(flow_out, 
"ATOM  %5s  %-3s %3s %c%4s    %8.3f%8.3f%8.3f%6.2f%6.2f          %c  \n",
atoms[i].AtomNumber, atoms[i].AtomName, 
atoms[i].ResType, atoms[i].Chain, 
atoms[i].ResNumber, atoms[i].XCoord, atoms[i].YCoord, 
atoms[i].ZCoord, atoms[i].Occup, atoms[i].B, atoms[i].AtomName[0]);
}
fclose(flow_out);
return 0;
}
...
unsigned int readerPDB (char *filename, unsigned int *dnanum, unsigned int *protnum, 
              unsigned int maxnumber, struct atom **patoms_dna, char **chains_dna, unsigned int *chains_num,
              struct atomname **plist)
{
  ...
  while( atomflag && (strcmp(currentatom.ModelNumber, CurrentModel) == 0 ) ){
    ...
    if( strcmp(currentatom.ModelNumber, CurrentModel) == 0 ) {
          if ( ifdna(currentatom.ResType) == TRUE ) {
             (*dnanum)++;
          ...
          } /* if the atom is from protein / else if it is from dna */
