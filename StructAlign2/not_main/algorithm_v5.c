...
{
  char *infile1, *infile2, *outfile, *max_score_name;
  char chain1, chain2;
  ...
  max_score_name = (char *)malloc( sizeof(char)*(strlen(argv[6])+1) );
  sscanf(argv[6], "%s", max_score_name);
  ...
  /*** For server ***/
  max_score_filename = (char *)malloc( sizeof(char)*(strlen(max_score_name)+31+1) );
  strcpy(max_score_filename, "");
  strcat(max_score_filename, max_score_name);
printf("%s\n", max_score_filename);
